...
var FCK = {Name:FCKURLParams['InstanceName'],Status:0,EditMode:0,Toolbar:null,HasFocus:false,DataProcessor:new FCKDataProcessor(),GetInstanceObject:(function(){
	...
	while(D){
		var F = false;
		switch(D.nodeType){case 1:var G = D.nodeName.toLowerCase();if(!FCKListsLib.BlockElements[G] && G != 'li' && !D.getAttribute('_fckfakelement') && D.getAttribute('_moz_dirty') == null){
			F = true;
		}break;case 3:if(E || D.nodeValue.Trim().length > 0){
			F = true;
		}break;case 8:if(E){
			F = true;
		}break;
...
FCKTools.CreateXmlObject = function(A){
	var B;
	switch(A){case 'XmlHttp':if(document.location.protocol != 'file:'){
		try{
			return new XMLHttpRequest();
		} catch (e){
		}
	}B = ['MSXML2.XmlHttp','Microsoft.XmlHttp'];break;case 'DOMDocument':B = ['MSXML2.DOMDocument','Microsoft.XmlDom'];break;
	}
	for(var i = 0; i < 2; i++){
		try{
...
FCKXHtml._RemoveXHtmlJobProperties = function (A){
	...
	if(typeof A._fckxhtmljob !== 'undefined'){
		A.removeAttribute('_fckxhtmljob');
	}
	if(A.hasChildNodes()){
		var B = A.childNodes;
		for(var i = B.length - 1; i >= 0; i--){
			FCKXHtml._RemoveXHtmlJobProperties(B.item(i));
		}
	}
};
					...
					if(P == this.Element){
						for(var Q in E){
							if(FCKDomTools.HasAttribute(O, Q)){
								switch(Q){case 'style':this._RemoveStylesFromElement(O);break;case 'class':if(FCKDomTools.GetAttributeValue(O, Q) != this.GetFinalAttributeValue(Q)){
									continue;
								}default:FCKDomTools.RemoveAttribute(O, Q);
								}
							}
						}
					}
			...
			if(c){
				for(var Q in E){
					if(FCKDomTools.HasAttribute(Z, Q)){
						switch(Q){case 'style':this._RemoveStylesFromElement(Z);break;case 'class':if(FCKDomTools.GetAttributeValue(Z, Q) != this.GetFinalAttributeValue(Q)){
							continue;
						}default:FCKDomTools.RemoveAttribute(Z, Q);
						}
					}
				}
			} else {
	...
	for(var i = D.length - 1; i >= 0; i--){
		...
		for(var F in B){
			if(FCKDomTools.HasAttribute(E, F)){
				switch(F){case 'style':this._RemoveStylesFromElement(E);break;case 'class':if(FCKDomTools.GetAttributeValue(E, F) != this.GetFinalAttributeValue(F)){
					continue;
				}default:FCKDomTools.RemoveAttribute(E, F);
				}
			}
		}
...
var FCKListHandler = {OutdentListItem:function(A){
	...
	var B = A.firstChild;
	while(B){
		switch(B.nodeType){case 1:if(!B.nodeName.IEquals('UL', 'LI')){
			return true;
		}break;case 3:if(B.nodeValue.Trim().length > 0){
			return true;
		}
		}
		B = B.nextSibling;
...
FCKDomRange.prototype = {_UpdateElementInfo:function(){
	...
	while(F){
		switch(F.nodeType){case 1:if(!FCKListsLib.InlineChildReqElements[F.nodeName.toLowerCase()]){
			return A.IsStartOfBlock = false;
		}break;case 3:if(F.nodeValue.Trim().length > 0){
			return A.IsStartOfBlock = false;
		}
		}
	...
	var H = false;
	while(F){
		switch(F.nodeType){case 1:var I = F.nodeName.toLowerCase();if(FCKListsLib.InlineChildReqElements[I]){
			break;
		}if(I == 'br' && !H){
			H = true;
			break;
		}return this._Cache.IsEndOfBlock = false;case 3:if(F.nodeValue.Trim().length > 0){
			return this._Cache.IsEndOfBlock = false;
		}
		}
	...
	}break;case 'block_contents':case 'list_contents':var D = FCKListsLib.BlockBoundaries;if(A == 'list_contents' || FCKConfig.EnterMode == 'br'){
...
FCKSelection.MoveToAncestorNode = function(A){
	...
	if(this.GetSelection().type == "Control"){
		...
		for(i = 0; i < oRange.length; i++){
			if(oRange.item(i).parentNode){
				B = oRange.item(i).parentNode;
				break;
			}
		}
	} else {
...
FCKSelection.Save = function(A){
	...
	if(C){
		...
		if(D){
			if(D.parentElement && FCKTools.GetElementDocument(D.parentElement()) != B){
				D = null;
...
FCKVisitLinkCommand.prototype = {GetState:function(){
	if(FCK.EditMode != 0){
		return -1;
	}
...
FCKDeleteDivCommand.prototype = {GetState:function(){
	if(FCK.EditMode != 0){
		return -1;
	}
...
FCKNbsp.prototype = {Execute:function(){
	FCK.InsertHtml('&nbsp;');
},GetState:function(){
...
FCKSpellCheckCommand.prototype.Execute = function(){
	switch(FCKConfig.SpellChecker){case 'ieSpell':this._RunIeSpell();break;case 'SpellerPages':FCKDialog.OpenDialog('FCKDialog_SpellCheck', 'Spell Check', 'dialog/fck_spellerpages.html', 440, 480);break;case 'WSC':FCKDialog.OpenDialog('FCKDialog_SpellCheck', 'Spell Check', 'wsc/w.html', 530, 480);
	}
};
...
FCKTableCommand.prototype.GetState = function(){
	if(FCK.EditorDocument != null && FCKSelection.HasAncestorNode('TABLE')){
		switch(this.Name){case 'TableHorizontalSplitCell':case 'TableVerticalSplitCell':if(FCKTableHandler.GetSelectedCells().length == 1){
			return 0;
		} else {
			return -1;
		}case 'TableMergeCells':if(FCKTableHandler.CheckIsSelectionRectangular() && FCKTableHandler.GetSelectedCells().length > 1){
			return 0;
		} else {
			return -1;
		}case 'TableMergeRight':return FCKTableHandler.GetMergeRightTarget() ? 0 : -1;case 'TableMergeDown':return FCKTableHandler.GetMergeDownTarget() ? 0 : -1;default:return 0;
		}
	} else {
		return -1;
	}
...
(function(){
	...
	function ScaytEngineLoad(callback){
		...
		L._djScaytConfig = {I:K,addOnLoad:function(){
			F();
		},isDebug:false};
		if(callback){
			A.push(callback);
	...
	T.prototype.Execute = function(c){
		switch(c){case 'Options':case 'Langs':case 'About':if(B && D && !C){
			ScaytMessage('SCAYT is not enabled');
			break;
		}if(B && D){
			FCKDialog.OpenDialog('Scayt', 'SCAYT Settings', 'dialog/fck_scayt.html?' + c.toLowerCase(), 343, 343);
		}break;default:if(!B){
			var U = this;
	...
	b.prototype.Execute = function(contextInfo){
		...
		if(g){
			switch(c){case 'Suggestion':Q.replace(g, contextInfo.suggestion);break;case 'Ignore':Q.ignore(g);break;case 'Ignore All':Q.ignoreAll(g);break;case 'Add Word':var E = FCK.EditorWindow.parent.parent;E.scayt.addWordToUserDictionary(g);break;
			}
		}
	};
	function InitSetup(){
		FCK.ContextMenu.RegisterListener({AddItems:function(menu){
			var E = FCK.EditorWindow.parent.parent;
			...
			for(var i = 0; i < k; i += 1){
				if(h[i]){
					menu.AddItem('ScaytContext', h[i], null, false, {'action':'Suggestion','node':g,'suggestion':h[i] });
				}
			}
			menu.AddSeparator();
			menu.AddItem('ScaytContext', 'Ignore', null, false, { 'action':'Ignore','node':g });
			menu.AddItem('ScaytContext', 'Ignore All', null, false, { 'action':'Ignore All','node':g });
			menu.AddItem('ScaytContext', 'Add Word', null, false, { 'action':'Add Word','node':g });
			try{
				if(D && C){
					Q.fireOnContextMenu(null, FCK.ContextMenu._InnerContextMenu);
				}
			} catch(err){
			}
		}});
	...
	FCKScayt = {CreateCommand:function(){
		return new T();
...
function FCKToolbarSet_Create(A){
	...
	switch(C){case 'In':document.getElementById('xToolbarRow').style.display = '';B = new FCKToolbarSet(document);break;case 'None':B = new FCKToolbarSet(document);break;default:FCK.Events.AttachEvent('OnBlur', FCK_OnBlur);FCK.Events.AttachEvent('OnFocus', FCK_OnFocus);var D;var E = C.match(/^Out:(.+)\((\w+)\)$/);if(E){
		if(FCKBrowserInfo.IsAIR){
	...
	}B = D.__FCKToolbarSet;if(B){
		break;
	}var F = FCKTools.GetElementDocument(D).createElement('iframe');F.src = 'javascript:void(0)';F.frameBorder = 0;F.width = '100%';F.height = '10';D.appendChild(F);F.unselectable = 'on';var G = F.contentWindow.document;var H = '';if(FCKBrowserInfo.IsSafari){
		H = '<base href="' + window.document.location + '">';
	}G.open();G.write('<html><head>' + H + '<script type="text/javascript"> var adjust = function() { window.frameElement.height = document.body.scrollHeight ; }; window.onresize = window.onload = function(){var timer = null;var lastHeight = -1;var lastChange = 0;var poller = function(){var currentHeight = document.body.scrollHeight || 0;var currentTime = (new Date()).getTime();if (currentHeight != lastHeight){lastChange = currentTime;adjust();lastHeight = document.body.scrollHeight;}if (lastChange < currentTime - 1000) clearInterval(timer);};timer = setInterval(poller, 100);}</script></head><body style="overflow: hidden">' + document.getElementById('xToolbarSpace').innerHTML + '</body></html>');G.close();if(FCKBrowserInfo.IsAIR){
		FCKAdobeAIR.ToolbarSet_InitOutFrame(G);
	}FCKTools.AddEventListener(G, 'contextmenu', FCKTools.CancelEvent);FCKTools.AppendStyleSheet(G, FCKConfig.SkinEditorCSS);B = D.__FCKToolbarSet = new FCKToolbarSet(G);B._IFrame = F;if(FCK.IECleanup){
...
function FCK_ContextMenu_GetListener(A){
	...
	}};case 'DivContainer':return {AddItems:function(menu, tag, tagName){
