...
sub create {
    ...
    my $entry = delete $opts{entry}
        or croak "No entry passed to create";
    ...
    my $sth = $dbh->prepare("INSERT INTO poll (itemid, journalid, posterid, whovote, whoview, name) " .
                            "VALUES (?, ?, ?, ?, ?, ?)");
    $sth->execute($entry->ditemid, $entry->journalid, $entry->posterid,
...
sub can_view {
    ...
    # must be person and visible to vote
    return 0 if $remote && ($remote->is_person || $remote->is_visible);
