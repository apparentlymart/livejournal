...
sub execute {
    ...
    if ($cmd eq 'list') {
        if ($pkg && $pkg ne 'all') {
            ...
            my $packages = $dbh->selectall_arrayref('SELECT pkgid, name, lastmoduserid, lastmodtime FROM priv_packages '
                                                    . 'ORDER BY name');
            return $self->error("Database error: " . $dbh->errstr)
                if $dbh->err;
    ...
    # create a package
        ...
        return $self->error("Package names can only contain letters, numbers, underscores, hyphens, or underscores.")
            unless $pkg =~ /^[a-z0-9_\-:\(\)\[\]]+$/i;

        $dbh->do("INSERT INTO priv_packages (pkgid, name, lastmoduserid, lastmodtime) "
    ...
    # add or remove a privilige to a package
        ...
        return $self->error("Argument must be in format of 'priv:arg' with optional arg.  (The colon is required.)")
        ...
        # exists or not
        my $exists = $dbh->selectrow_array('SELECT COUNT(*) FROM priv_packages_content '
