...
# internal fields:
#
#    journal_u:     u object,                        always present
...
sub new
{
    ...
    $self->{journal_u} = LJ::want_user($uuserid);
    croak("invalid user/journalid parameter")
        unless LJ::isu($self->{journal_u});

    $self->{jtalkid} = delete $opts{jtalkid};
...
sub preload_rows {
    ...
    foreach my $obj (@$obj_list) {
        ...
        my $u = $obj->journal_u;
        my $row = LJ::Talk::get_talk2_row
            ($obj->journal_u, $obj->journalid, $obj->jtalkid);

        next unless $row;
        for my $f (qw(journalid jtalkid nodetype nodeid parenttalkid posterid datepost state)) {
            $obj->{$f} = $row->{$f};
        }
        $obj->{_loaded_row} = 1;
    }
